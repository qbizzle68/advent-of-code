...
def parse_input(filepath: str) -> defaultdict[tuple[int, int], int]:
    ...
    # The defining difference in part 2 is the corner lights are always on.
    # Therefore if they're not on in the input, set them here.

    map_[(0, 0)] = 1
    map_[(0, cols - 1)] = 1
    map_[(rows - 1, 0)] = 1
    map_[(rows - 1, cols - 1)] = 1
...
def main(state: defaultdict[tuple[int, int], int], rows: int,
    ...
    for _ in range(iterations):
        ...
        for i in range(rows):
            for j in range(cols):
                # Corners are always on, so skip them
