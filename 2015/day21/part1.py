...
def main(items: dict[str, list[EquipableItem]], initial_state: dict[str, int]) -> int:
    ...
    winning_loadouts = [loadout for loadout in loadouts if play_battle(loadout, initial_state) is True]

    return min((sum(item.cost for item in loadout.items) for loadout in winning_loadouts))
