...
def rotate_characters(data: dict) -> list[tuple[str, int]]:
    values = []
    for d in data:
        line = d['line']
        id = d['id']
        base_values = [ord(c) - 0x61 for c in line]
        new_values = []
        for v in base_values:
            if v != -52:
                new_values.append(chr((v + id) % 26 + 0x61))
            else:
                new_values.append(' ')
        values.append((''.join(new_values), id))

    return values


def main(data: dict) -> int:
    strings = rotate_characters(data)
    for s, id_ in strings:
        if 'object' in s:
            return id_
